...
ig.Director = ig.Class.extend({
  ...
  nextLevel: function(disableFade){
	//Advance to the next level provided we are not at 
	//the end of the array.
	if (this.currentLevel + 1 < this.levels.length){	
		
		return this.loadLevel(this.currentLevel + 1, disableFade);
	}
	else{
	  return false;
	}
  ...
  previousLevel: function(disableFade){
  ...
  jumpTo: function(requestedLevel, disableFade){
	...
	if (levelNumber >= 0){
	  return this.loadLevel(levelNumber, disableFade);
	}
	else{
	  return false;
	}
